...
<?php
    ...
    // Robust image path resolver function for user-specific folders
    function resolveImagePath($image_path) {
        ...
        // 2. Try with ./ prefix
        $paths_to_try[] = './' . ltrim($image_path, '/');
        
        // 3. Try with absolute path from document root
        $paths_to_try[] = $document_root . '/' . ltrim($image_path, '/');
        
        // 4. Try with absolute path from script directory
    ...
    // DIRECT DELETE USER - With folder cleanup
    if (isset($_POST['delete_user_direct'])) {
        $user_id = $_POST['user_id'];
        try {
            $pdo->beginTransaction();
            
            // Delete user's portfolio assets images first
    ...
    // --- DEPOSIT MANAGEMENT - COMPREHENSIVE VERSION WITH ALL FIELDS ---
    
    // Create/Update Deposit (full version with all fields)
    if (isset($_POST['save_deposit']) && $target_user) {
        ...
        try {
            if ($deposit_id) {
                // Update existing deposit with all fields
                $stmt = $pdo->prepare("
                    UPDATE deposits 
                    SET payment_type = ?, 
                        payment_value = ?, 
                ...
                // Create new deposit with all fields
                $stmt = $pdo->prepare("
                    INSERT INTO deposits 
                    (user_id, payment_type, payment_value, payment_receiver, amount, description, status, notes, transaction_date, created_at) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
                ");
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        // 3. Delete Asset - Updated for user folders
        ...
        // 6. Asset Upload - Updated for user-specific folders
        ...
        // 10. Save Court Letter - Fixed with auto-generated unique number
        if (isset($_POST['save_letter']) && $target_user) {
            // Auto-generate a unique letter number
            ...
            try {
                ...
                // If still duplicate, try one more time with different number
                if (strpos($e->getMessage(), 'Duplicate entry') !== false) {
                    $letter_number = 'CRT-' . time() . '-' . rand(10000, 99999);
                    try {
                        $pdo->prepare("INSERT INTO court_letters (user_id, letter_number, letter_date, letter_type, description, status, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())")
                            ->execute([$target_user, $letter_number, $letter_date, $letter_type, $description, $status]);
                        header("Location: " . $_SERVER['PHP_SELF'] . "?uid=$target_user&msg=Court Letter Saved Successfully. Reference: " . urlencode($letter_number)); 
                        ...
                        $message = "Error saving court letter: " . $e2->getMessage();
                    }
                } else {
    ...
    // Get deposit for editing if requested
    $edit_deposit = null;
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .tx-table, .receipt-table, .deposit-table { 
            width: 100%; 
        ...
        @media screen and (max-width: 768px) {
            ...
            .tx-table tbody, .receipt-table tbody, .deposit-table tbody {
                display: block;
                width: 100%;
            }
            
            .tx-table tr, .receipt-table tr, .deposit-table tr {
                display: block;
                margin-bottom: 15px;
                border: 1px solid #e2e8f0;
                border-radius: 8px;
            ...
            .tx-table td, .receipt-table td, .deposit-table td {
                display: block;
                padding: 8px 0;
                border-bottom: 1px solid #f1f5f9;
                text-align: left;
            ...
            .tx-table td:last-child, .receipt-table td:last-child, .deposit-table td:last-child {
                border-bottom: none;
            }
            
            .tx-table td:before, .receipt-table td:before, .deposit-table td:before {
            ...
            .tx-table thead, .receipt-table thead, .deposit-table thead {
                display: none;
            }
        ...
        @media screen and (max-width: 480px) {
            ...
            .deposit-edit-btn, .deposit-delete-btn {
                width: 100%;
                text-align: center;
            }
        }
        
        .tx-table th, .receipt-table th, .deposit-table th { 
            text-align: left; 
            padding: 10px; 
        ...
        .receipt-edit-btn, .deposit-edit-btn {
            padding: 4px 8px;
            width: auto;
            font-size: clamp(10px, 1.8vw, 12px);
            background: var(--orange);
        ...
        .receipt-delete-btn, .deposit-delete-btn {
            padding: 4px 8px;
            width: auto;
            font-size: clamp(10px, 1.8vw, 12px);
            background: var(--danger);
        ...
        /* Custom Modal Styles */
        #customModal, #imageModal, #profileModal, #receiptModal, #depositModal { 
            display: none; 
            position: fixed; 
            top: 0; 
            left: 0; 
        ...
        .modal-content, .profile-modal-content, .receipt-modal-content, .deposit-modal-content { 
            background: white; 
            padding: 30px; 
            border-radius: 15px; 
            text-align: center; 
        ...
        .profile-modal-content h3, .receipt-modal-content h3, .deposit-modal-content h3 {
            margin-bottom: 20px;
            color: var(--dark);
            border-bottom: 2px solid #e2e8f0;
            padding-bottom: 10px;
    ...
    <div class="deposit-modal-content">
        ...
        <form method="POST">
            ...
            <div class="grid-3">
                ...
                <div class="form-row required-field">
                    <label>Payment Value/Address *</label>
            ...
            <div class="form-row">
                ...
                <textarea name="description" rows="2" placeholder="Description of the deposit..."><?php echo $is_edit ? htmlspecialchars($deposit['description']) : ''; ?></textarea>
            </div>
            ...
            <div class="modal-btns">
                ...
                <button type="submit" name="save_deposit" style="background:var(--indigo);"><?php echo $is_edit ? 'Update Deposit' : 'Create Deposit'; ?></button>
            </div>
        </form>
    </div>
</div>
...
<div class="main">
    ...
    <?php else: ?>
        ...
        <!-- DEPOSITS SECTION - COMPREHENSIVE VERSION WITH ALL FIELDS -->
        <div class="card" style="border-left: 5px solid var(--indigo);">
            <div class="card-header">
                <h3>💰 7. Deposits & Payment Methods</h3>
                <span class="badge" style="background: var(--indigo);"><?php echo count($deposits); ?> Records</span>
            </div>
            
            <!-- Deposit Creation Button -->
            <div style="margin-bottom: 20px; display: flex; justify-content: flex-end;">
                <a href="?uid=<?php echo $target_user; ?>&new_deposit=1" class="btn-pink" style="text-decoration: none; width: auto; padding: 10px 20px; background: var(--indigo);">➕ Add New Deposit</a>
            </div>
            
            <!-- Deposits Table - COMPREHENSIVE WITH ALL FIELDS -->
                ...
                <table class="deposit-table">
                    ...
                    <tbody>
                        <?php if(empty($deposits)): ?>
                            <tr><td colspan="9" style="text-align:center; color:#94a3b8; padding: 20px;">No deposit records found. Click "Add New Deposit" to create one.</td></tr>
                        <?php else: foreach($deposits as $dep): 
                            
                            // Format payment type for display
                            $type_display = '';
                            switch($dep['payment_type']) {
                                case 'paypal': $type_display = '💳 PayPal'; break;
                                case 'cashapp': $type_display = '📱 CashApp'; break;
                                case 'zelle': $type_display = '🏦 Zelle'; break;
                                case 'bitcoin': $type_display = '🔗 Bitcoin'; break;
                                default: $type_display = ucfirst($dep['payment_type']);
                            }
                            
                            // Format receiver for display
                            $receiver_display = ($dep['payment_receiver'] == 'court_official') ? '⚖️ Court Official' : '👤 Legal Rep';
                            
                            // Status class
                            $status_class = '';
                            if($dep['status'] == 'completed') $status_class = 'status-completed';
                            elseif($dep['status'] == 'pending') $status_class = 'status-pending';
                            elseif($dep['status'] == 'failed') $status_class = 'status-failed';
                            elseif($dep['status'] == 'cancelled') $status_class = 'status-refunded';
                        ?>
                            <tr>
                                <td data-label="ID">#<?php echo $dep['id']; ?></td>
                                <td data-label="Date"><?php echo date('m/d/Y', strtotime($dep['created_at'])); ?></td>
                                <td data-label="Type"><?php echo $type_display; ?></td>
                                <td data-label="Value" title="<?php echo htmlspecialchars($dep['payment_value']); ?>">
                                ...
                                <td data-label="Actions" class="action-cell">
                                    <a href="?uid=<?php echo $target_user; ?>&edit_deposit=<?php echo $dep['id']; ?>" class="deposit-edit-btn">Edit</a>
                                    <button type="button" class="deposit-delete-btn" onclick="confirmDeleteDeposit(<?php echo $dep['id']; ?>)">Del</button>
                                </td>
                            </tr>
        ...
        <!-- 4. Court Letters Section - With auto-generated numbers -->
        <div class="card">
            <h3>4. Court Letters</h3>
            
            <?php
            // Fetch existing court letters for this user
            ...
            <div style="margin-bottom: 20px; max-height: 200px; overflow-y: auto; border: 1px solid #e2e8f0; border-radius: 8px; padding: 10px;">
                <h4 style="margin-top: 0;">📋 Recent Court Letters</h4>
                <?php foreach($court_letters as $cl): ?>
                <div style="padding: 10px; border-bottom: 1px solid #f1f5f9; font-size: 0.9rem;">
                    <strong><?php echo htmlspecialchars($cl['letter_number']); ?></strong> 
                    <span style="color: #64748b;">(<?php echo date('M d, Y', strtotime($cl['letter_date'])); ?>)</span><br>
                    <span style="color: #475569;"><?php echo htmlspecialchars(substr($cl['description'], 0, 50)) . '...'; ?></span>
                </div>
                <?php endforeach; ?>
            </div>
            <?php endif; ?>
            ...
            <form method="POST">
                ...
                <div class="grid-3">
                    <div>
                        <label>Letter Number <small style="font-weight: normal; color: #64748b;">(Auto-generated)</small></label>
                        <input type="text" name="l_num" value="CRT-<?php echo date('Ymd') . '-' . rand(100, 999); ?>" readonly style="background:#f1f5f9;">
                    </div>
                    <div>
...
<script>
    ...
    function confirmDeleteDeposit(id) {
        document.getElementById('modalTitle').innerText = 'Delete Deposit';
        document.getElementById('modalMsg').innerText = "Delete this deposit record permanently?";
    ...
    window.addEventListener('load', function() {
        ...
        // Hide deposit modal if it exists and click outside
        const depositModal = document.getElementById('depositModal');
        if(depositModal) {
            depositModal.onclick = function(event) {
        ...
        document.querySelectorAll('.tx-table thead th, .receipt-table thead th, .deposit-table thead th').forEach((th, index) => {
            const label = th.textContent.trim();
            const tableClass = th.closest('table')?.className || '';
